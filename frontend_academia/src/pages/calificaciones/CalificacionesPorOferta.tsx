...
import { listarCalificacionesPorOferta } from '../../services/calificaciones'
import { uuidSchema } from '../../validation/schemas'

const columns: TableColumn<Calificacion>[] = [
  { key: 'inscripcionId', header: 'Inscripción' },
...
const CalificacionesPorOferta = () => {
  ...
  const {
    ...
    queryKey: ['calificaciones', 'oferta', ofertaId],
    queryFn: () => listarCalificacionesPorOferta(ofertaId!),
  ...
  return (
    <Page
      title="Calificaciones por oferta"
      description="Consulta las calificaciones emitidas para una oferta de curso."
      actions={
        <Button
          variant="secondary"
          onClick={() => refetch()}
    ...
    >
      ...
      {error ? (
        ...
        <Table
          className="mt-6"
          data={ofertaId ? calificaciones : []}
          columns={columns}
          emptyMessage={
            ofertaId
              ? 'No hay calificaciones registradas para la oferta seleccionada.'
              : 'Ingresa una oferta para visualizar sus calificaciones.'
          }
...
export default CalificacionesPorOferta
